...
import { ItemCarrinho } from "@/data/models"
...
export default function ItemCarrinhoCard(props: ItemCarrinhoCardProps) {
	...
	return (
		<div className="flex gap-5 bg-black rounded-md p-2">
			<span>{item.produto.nome}</span>
			<div className="flex items-center gap-2">
				<button
					className="flex justify-center items-center h-5 w-5 bg-red-500 rounded-sm"
				...
				<button
					className="flex justify-center items-center h-5 w-5 bg-green-500 rounded-sm"
					onClick={() => adicionarItem(item.produto)}
				>
					<IconPlus />
				</button>
			</div>
			<div className="bg-blue-500 rounden-md px-2 font-black">
